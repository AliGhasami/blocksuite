...
export class MahdaadImageService extends BlockService<MahdaadImageBlockModel> {
  ...
  private _dragHandleOption: DragHandleOption = {
    flavour: MahdaadImageBlockSchema.model.flavour,
    edgeless: true,
    onDragStart: ({ state, startDragging, anchorBlockPath, editorHost }) => {
      ...
      if (
        ...
        !matchFlavours(anchorComponent.model, [
          MahdaadImageBlockSchema.model.flavour,
        ])
      )
        return false;

      const blockComponent = anchorComponent as MahdaadImageBlockComponent;
    ...
    onDragEnd: props => {
      ...
      const blockComponent = draggingElements[0] as MahdaadImageBlockComponent;
      const isInSurface = blockComponent.isInSurface;
