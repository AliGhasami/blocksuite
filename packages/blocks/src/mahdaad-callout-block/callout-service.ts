...
export class MahdaadCalloutBlockService extends BlockService {
  static override readonly flavour = MahdaadCalloutBlockSchema.model.flavour;
  ...
  private _getClosestNoteByBlockId = (blockId: string) => {
    ...
    while (parent) {
      if (matchFlavours(parent, [MahdaadCalloutBlockSchema.model.flavour])) {
        return parent;
      }
...
export const calloutDragHandleOption = DragHandleConfigExtension({
  ...
  onDragEnd: ({
    ...
    if (
      ...
      !matchFlavours(draggingElements[0].model, [MahdaadCalloutBlockSchema.model.flavour])
    ) {
      return false;
    }
