...
export class MahdaadMultiColumnBlockComponent extends CaptionedBlockComponent<MahdaadMultiColumnBlockModel> {
  ...
  addColumn(currentIndex:number,position:'left'|'right') {
    ...
    const ids= this.doc.addSiblingBlocks(temp,[
      { flavour: 'affine:note' },
  ...
  moveColumn(currentIndex:number,position:'left'|'right') {
    const target=position=='left' ? this.model.children[currentIndex-1] : this.model.children[currentIndex+1]
    this.doc.moveBlocks([this.model.children[currentIndex]],this.model,target,position=='left')
  }
