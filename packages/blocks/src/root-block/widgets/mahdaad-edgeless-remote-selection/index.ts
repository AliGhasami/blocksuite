...
export const MAHDAAD_EDGELESS_REMOTE_SELECTION_WIDGET =
  'mahdaad-edgeless-remote-selection-widget';

@customElement(MAHDAAD_EDGELESS_REMOTE_SELECTION_WIDGET)
export class MahdaadEdgelessRemoteSelectionWidget extends WidgetComponent<
  RootBlockModel,
...
> {
  ...
  private _updateRemoteCursor = () => {
    const remoteCursors: MahdaadEdgelessRemoteSelectionWidget['_remoteCursors'] =
      new Map();
  ...
  private _updateRemoteRects = () => {
    ...
    const remoteRects: MahdaadEdgelessRemoteSelectionWidget['_remoteRects'] =
      new Map();
      ...
      ([id, cursor]) => {
        return html`<div
          ...
          style=${styleMap({
            ...
            color: cursor.user?.color ?? _remoteColorManager.get(id),
          })}
        >
          ${RemoteCursor}
          <div
            ...
            style=${styleMap({
              backgroundColor:
                cursor.user?.color ?? _remoteColorManager.get(id),
            })}
          >
            <mahdaad-doc-selection-cursor
              user-id="${cursor.user?.user_id}"
            ></mahdaad-doc-selection-cursor>
          </div>
        </div>`;
      }
...
declare global {
  interface HTMLElementTagNameMap {
    [MAHDAAD_EDGELESS_REMOTE_SELECTION_WIDGET]: MahdaadEdgelessRemoteSelectionWidget;
  }
}
