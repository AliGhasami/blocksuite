    ...
    <div class="flex">
      <div class="calendar">
        <div class="month-year-container">
          ...
          >
            <SelectOption v-for="val in getYears(currentDate)" :key="String(val)" class="year-item">
              {{ val }}
            </SelectOption>
          </Select>
        </div>
...
function handleOKTime() {
  ...
  emit('close')
  /*if (thisRef.value) {
    const okBtn: HTMLElement | null = (thisRef.value as HTMLElement).querySelector(
...
.mahdaad-date-picker {
  ...
  :where(.css-dev-only-do-not-override-19iuou).ant-picker-calendar
    .ant-picker-panel
